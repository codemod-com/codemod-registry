...
// auto imported from https://github.com/mskelton/ratchet/tree/main/__testfixtures__
describe.only('ratchet', function () {
	...
	it('complex-props', function () {
		...
		const actualOutput = transform(fileInfo, buildApi('tsx'), { "preserve-prop-types": "unconverted" });
		assert.deepEqual(
			actualOutput?.replace(/\W/gm, ''),
			OUTPUT.replace(/\W/gm, ''),
		);
	...
	it('preserve-prop-types', function () {
		...
		const actualOutput = transform(fileInfo, buildApi('tsx'), { "preserve-prop-types": "all" });
		assert.deepEqual(
			actualOutput?.replace(/\W/gm, ''),
			OUTPUT.replace(/\W/gm, ''),
		);
