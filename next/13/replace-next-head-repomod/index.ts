...
/**
 * Copypasted from 'replace-next-head'
 */
...
const handleHeadChildJsxElement = (
	...
	children.forEach((child) => {
		...
		console.log(child.getText(), 'child');
...
const handleHeadJsxElement = (
	...
	console.log(jsxChildren.length, 'test11');
...
const isIdentifierDefinedInPageProps = (identifier: Identifier): boolean => {
	...
	console.log('isIdentifierDefinedInPageProps');
	console.log(
		definitions.map((d) => d.getText()),
		'defs',
	);
	...
	return (
		...
		definitions.every((d) => {
			...
			// @TODO this will not track the arrow functions
			return (
				param &&
...
export const handleSourceFile = (
	...
	console.log(importDeclarations.length);
	importDeclarations.forEach((importDeclaration) =>
	...
	treeNode.metadata = { ...metadataObject };
...
const resolveDependencies = async (
	...
	// collects metadata
	handleSourceFile(sourceFile, treeNode);
	...
	for (const path of paths) {
		...
		identifiers.forEach((identifier) => {
			...
			if (jsxElement !== undefined) {
				...
				treeNode.components[resolvedFileName].props = {};
...
const recur = async (
	...
	const node = await resolveDependencies(tsmorph, containingPath, treeNode);

	Object.keys(node.components).forEach((containingPath) => {
		recur(tsmorph, containingPath, node.components[containingPath]);
	});

	return node;
};

const mergeMeta = (treeNode: any) => {
...
export const repomod: Repomod<Dependencies> = {
	...
	handleFile: async (api, path) => {
		...
		await recur(tsmorph, path, componentsTree);
		
		console.log(JSON.stringify(componentsTree, null, 2));
		return [];
	},
