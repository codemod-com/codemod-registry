...
function isCustomValidator(path: any) {
  return (
...
const resolveRequired = (path: any) =>
  isRequired(path) ? path.get("object") : path

function getTSType(path: any) {
  const { value: name } =
...
const isRequired = (path: any) =>
  path.get("type").value === "MemberExpression" &&
  path.get("property", "name").value === "isRequired"

function mapType(path: any): TSType {
...
function getTSTypes(
  source: Collection,
  getComponentName: (path: any) => string
) {
 ...
 */
function addForwardRefTypes(path: any, typeName: string): boolean {
