...
export default function transformer(file, api, options) {
	...
	function getPrefix(withStylesCall) {
		...
		if (
			withStylesCall &&
			withStylesCall.arguments[1] &&
			withStylesCall.arguments[1].properties
		) {
			const name = withStylesCall.arguments[1].properties.find(
			...
			root.findJSXElements(name)
				...
				.forEach((path) => {
					...
					if (existingClassName) {
						...
						if (
							existingClassName.value.type ===
							'JSXExpressionContainer'
						) {
							if (
								existingClassName.value.expression.type ===
								'StringLiteral'
							) {
								// className={'string'}
							}
							...
							if (
								...
								if (
									existingClassName.value.expression.object
										.name === 'classes'
								) {
									rootClassKeys.push(
										existingClassName.value.expression
											.property.name,
									);
								}
							}
							...
							if (
								...
								existingClassName.value.expression.arguments.forEach(
									(arg) => {
	...
	function createStyledComponent(
		...
		const declaration = j.variableDeclaration('const', [
			j.variableDeclarator(
				...
				j.callExpression(
					j.callExpression(j.identifier('styled'), [styleArg]),
					[stylesFn],
				),
			),
		]);
	...
	function convertToStyledArg(functionExpression, rootKeys, prevStyleArg) {
		...
		if (functionExpression.params) {
			functionExpression.params = functionExpression.params.map(
				(param) => {
					if (param.type === 'ObjectPattern') {
						return j.objectPattern([
							j.objectProperty(j.identifier('theme'), param),
						]);
					}
		...
		if (prevStyleArg) {
			...
			if (
				...
				prevStyleArg.type === 'ObjectExpression'
			) {
				// turn prevStyleArg to ArrowFunction
				prevStyleArg = j.arrowFunctionExpression(
					functionExpression.params,
					prevStyleArg,
				);
			}
	...
	const withStylesCall = root
		.find(j.CallExpression, { callee: { name: 'withStyles' } })
		.nodes()[0];
	const makeStylesCall = root
	...
	if (withStylesCall) {
		let stylesFnName;
		root.find(j.CallExpression, { callee: { name: 'withStyles' } }).forEach(
			(path) => {
				...
				if (objectExpression) {
					...
					result.classes = createClasses(
						objectExpression,
						result.classes,
					);
					result.styledArg = convertToStyledArg(
						arg,
						rootClassKeys,
						result.styledArg,
					);
		...
		root.find(j.VariableDeclarator, { id: { name: stylesFnName } })
			.forEach((path) => {
		...
		root.find(j.FunctionDeclaration, { id: { name: stylesFnName } })
			.forEach((path) => {
	...
	if (makeStylesCall) {
		...
		root.find(j.VariableDeclaration)
			.filter((path) =>
				path.node.declarations.some((d) => d.id.name === 'useStyles'),
			)
			.remove();
	}
	...
	if (rootJsxName === '') {
		...
		root.findJSXElements(rootJsxName)
			.at(0)
			.forEach(transformJsxRootToStyledComponent);
	}
	...
	withStylesComponents.forEach((data) => {
		root.find(j.JSXOpeningElement, {
			...
			path.node.attributes.push(
				j.jsxAttribute(
					j.jsxIdentifier('classes'),
					j.jsxExpressionContainer(data.classes),
				),
			);
		});
	});
	...
	const imports = root
		...
		.forEach(({ node }) => {
			const existed = node.specifiers.find(
				(s) => s.imported.name === 'styled',
			);
			if (!existed) {
	...
	root.find(j.CallExpression, { callee: { name: 'withStyles' } }).forEach(
		(path) => {
			if (
				...
				path.parent.parent.parent.parent.node.type !==
					'ExportNamedDeclaration' &&
				path.parent.node.arguments[0].type === 'Identifier'
			) {
				path.parent.parent.node.init = j.identifier(
					path.parent.node.arguments[0].name,
				);
			}
		},
	);
