...
export default function transformer(file, api, options) {
	...
	return j(file.source)
		...
		.forEach((path) => {
			...
			const subPackageImportMatch = importSource.match(
				/@material-ui\/lab\/(.*)/,
			);
			if (subPackageImportMatch !== null) {
				...
				if (movedLabModules.includes(componentName)) {
					...
					sourcePath.replace(
						j.stringLiteral(`@material-ui/core/${componentName}`),
					);
				}
				...
				if (coreImportSpecifiers.length > 0) {
					path.replace(
						j.importDeclaration(
							...
							j.stringLiteral('@material-ui/core'),
						),
						j.importDeclaration(
							labImportSpecifiers,
							j.stringLiteral('@material-ui/lab'),
						),
					);
				}
			}
