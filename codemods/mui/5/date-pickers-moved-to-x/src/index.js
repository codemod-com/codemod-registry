...
const buildLookup = () => {
	return Object.fromEntries(
		Object.entries(_exports).flatMap(([entryPoint, entryPointData]) =>
			[entryPointData.default, ...entryPointData.named].map(
				(exportName) => [
					exportName,
		...
		.forEach((path) => {
			...
			if (subPackageImportMatch !== null) {
				...
				if (subModule.startsWith('internal')) {
					console.warn(
						'Imports from `@mui/lab/internal` are not supported',
					);
					return;
				}
