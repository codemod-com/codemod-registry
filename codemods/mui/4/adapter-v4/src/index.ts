...
export default function transformer(
	 ...
	 */
	root.find(j.CallExpression, { callee: { name: 'createMuiTheme' } }).forEach(
		({ node }) => {
			if (hasAdaptV4(node)) {
				adaptV4Called = true;
			}
	 ...
	 */
	root.find(j.CallExpression, { callee: { name: 'createTheme' } }).forEach(
		({ node }) => {
			if (hasAdaptV4(node)) {
				adaptV4Called = true;
			}
			...
			.forEach(({ node }) => {
				if (
					!node.specifiers?.find(
						({ imported }) => imported.name === 'adaptV4Theme',
					)
				) {
					node.specifiers = [
						...node.specifiers,
