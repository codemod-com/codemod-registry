    ...
    root.find(j.CallExpression)
        .forEach(path => {
            ...
            if (path.node.callee.type === 'MemberExpression') {
                ...
                if (path.node.callee.object.type === 'Identifier' && path.node.callee.object.name === 'integration') {
                    // Ensure the property is an Identifier named 'addBuildHook'
                    if (path.node.callee.property.type === 'Identifier' && path.node.callee.property.name === 'addBuildHook') {
